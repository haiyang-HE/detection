...
classes = ["R-EPcell", "G-EPcell", "Neg-EPcell", "Db-EPcell"]

base_path = r'D:\data\data_guo\zdata\savedata617\val'
...
for path in path_list:
    ...
    with open(f'{base_path}/{path}.txt', 'w+') as f:
        for idx, mask_data in enumerate(masks):
            ...
            mask = np.array([np.array(i) for i in mask_data['points']], dtype=np.float32)
            mask[:, 0] /= w
            mask[:, 1] /= h
            mask = mask.reshape((-1))
            if idx != 0:
