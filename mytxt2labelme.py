...
def get_shapes(txt_path, width, height, class_labels):
    ...
    for shape in shapes:
        ...
        class_id = values[0]
        r_shape = dict()
        r_shape["label"] = class_labels[int(float(class_id))]
...
def yolo2labelme(data, out=None, skip=False):
    # class_labels = ["red blood cell","trophozoite" ,"ring", "difficult", "schizont", "gametocyte", "leukocyte"]
    class_labels = ["cell"]
    if out is None:
        out = os.path.join(os.path.abspath(data),'..','labelmeDataset')
    for dir_type in ['images']:
        dir_path = os.path.join(data, dir_type)
...
def main():
    ...
    argparser.add_argument('--data',default=r'D:\data\data_guo\DSB2018-2\val')
    argparser.add_argument('--out', default='val0000000', required=False)
